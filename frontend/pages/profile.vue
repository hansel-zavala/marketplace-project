    ...
    <div class="container mx-auto px-4">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div class="md:col-span-2 space-y-6">
          ...
          <div class="bg-white p-6 rounded-lg shadow-md relative">
            ...
            <div class="flex flex-col sm:flex-row items-center gap-6 mb-8">
              <div class="relative group w-24 h-24 shrink-0">
                <div class="w-24 h-24 rounded-full overflow-hidden bg-gray-200 border-4 border-white shadow-md flex items-center justify-center">
                  <img v-if="authStore.user?.profile_image" :src="getAvatarUrl(authStore.user.profile_image)" class="w-full h-full object-cover" />
              ...
              <div class="text-center sm:text-left">
                ...
                <p class="text-sm text-gray-400 mt-1">Miembro desde {{ formatDate(authStore.user?.created_at) }}</p>
              </div>
            </div>
            ...
            <div v-if="!isEditing" class="border-t pt-6">
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                  <label class="block text-xs uppercase tracking-wide text-gray-500 font-bold mb-1">Correo</label>
                  <p class="text-gray-800 font-medium">{{ authStore.user?.email }}</p>
                </div>
                <div>
                  <label class="block text-xs uppercase tracking-wide text-gray-500 font-bold mb-1">Teléfono</label>
            ...
            <form v-else @submit.prevent="saveChanges" class="border-t pt-6">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                  ...
                  <input v-model="form.first_name" type="text" class="w-full border rounded p-2 focus:ring-2 focus:ring-blue-500 outline-none">
                </div>
                <div>
                  <label class="block text-sm font-bold mb-1">Apellido</label>
                  <input v-model="form.last_name" type="text" class="w-full border rounded p-2 focus:ring-2 focus:ring-blue-500 outline-none">
                </div>
              </div>
              <div class="mb-6">
                <label class="block text-sm font-bold mb-1">Teléfono</label>
                <input v-model="form.phone" type="text" class="w-full border rounded p-2 focus:ring-2 focus:ring-blue-500 outline-none">
              </div>
              <div class="flex gap-3">
                <button type="submit" class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 font-medium" :disabled="loading">
                  {{ loading ? 'Guardando...' : 'Guardar Cambios' }}
                </button>
                <button type="button" @click="cancelEditing" class="bg-gray-200 text-gray-700 px-4 py-2 rounded hover:bg-gray-300 font-medium">
                  Cancelar
                </button>
              </div>
            </form>
          ...
          <div v-if="authStore.user?.user_type === 'professional' && professionalProfile" class="bg-white rounded-lg shadow-md overflow-hidden border border-blue-100">
            <div class="bg-blue-50 px-6 py-3 border-b border-blue-100 flex justify-between items-center">
              ...
              <span v-if="professionalProfile.verification_status === 'verified'" class="bg-green-100 text-green-700 text-xs px-2 py-1 rounded-full font-bold flex items-center gap-1">
                <BadgeCheck :size="14" /> Verificado
              </span>
              <span v-else class="bg-gray-200 text-gray-600 text-xs px-2 py-1 rounded-full font-bold">
                {{ professionalProfile.verification_status === 'pending' ? 'En Revisión' : 'No Verificado' }}
              </span>
            </div>
            ...
            <div class="p-6">
              <div class="flex justify-between items-start mb-4">
                <div>
                  <h4 class="text-xl font-bold text-gray-800">{{ professionalProfile.profession }}</h4>
                  <p v-if="professionalProfile.business_name" class="text-gray-500 text-sm">{{ professionalProfile.business_name }}</p>
                </div>
                <div class="text-right">
                  <p class="text-xs text-gray-500 uppercase font-bold">Tarifa</p>
                  <p class="text-lg font-bold text-gray-800">L. {{ professionalProfile.hourly_rate }} <span class="text-sm font-normal text-gray-500">/hr</span></p>
                </div>
              </div>
              ...
              <div class="flex items-center gap-6 text-sm text-gray-500 border-t pt-4">
                <div class="flex items-center gap-1">
                  ...
                  <span class="font-bold text-gray-700">{{ professionalProfile.rating || '0.0' }}</span>
                </div>
                <div class="flex items-center gap-1">
                  <Briefcase :size="16" />
              ...
              <div class="mt-6">
                <NuxtLink to="/professional" class="block w-full text-center border border-blue-600 text-blue-600 py-2 rounded-lg hover:bg-blue-50 font-medium transition">
        ...
        <div class="bg-white p-6 rounded-lg shadow-md h-fit">
          ...
          <ul class="space-y-2">
            <li>
              ...
              >
                <Briefcase :size="18" /> 
                {{ authStore.user?.user_type === 'professional' ? 'Panel Profesional' : 'Conviértete en Profesional' }}
              </NuxtLink>
            </li>
            <li>
              <NuxtLink to="/addresses" class="block w-full text-left px-4 py-2 rounded hover:bg-gray-50 text-gray-600 transition flex items-center gap-2">
...
import { 
  User, Edit2, LogOut, Camera, Loader2, 
  Briefcase, Map, Key, BadgeCheck, Star, MapPin 
...
onMounted(async () => {
  if (authStore.user?.user_type === 'professional') {
    try {
...
const formatDate = (dateString) => {
  ...
  return new Date(dateString).toLocaleDateString('es-HN', { year: 'numeric', month: 'long', day: 'numeric' });
};
