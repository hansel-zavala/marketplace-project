    ...
    <div class="container mx-auto px-4">
      ...
      <div v-else-if="!business" class="max-w-3xl mx-auto">
        <div class="text-center mb-10">
          ...
          <p class="text-gray-600 mt-2 text-lg">Crea tu tienda oficial para vender productos y llegar a más clientes.</p>
        </div>

        <div class="bg-white p-8 rounded-xl shadow-lg border border-gray-100">
          <BusinessForm 
            :loading="saving" 
            btn-label="Crear mi Tienda"
            @submit="saveBusiness" 
          />
        </div>
      </div>
      ...
      <div v-else class="max-w-6xl mx-auto">
        ...
        <div class="flex justify-between items-center mb-8">
          ...
          <div class="flex gap-3">
             <NuxtLink :to="`/businesses/${business.id}`" class="bg-white border border-gray-300 text-gray-700 px-4 py-2 rounded-lg hover:bg-gray-50 font-medium flex items-center gap-2 transition">
                <ExternalLink :size="18" /> Ver como Cliente
             </NuxtLink>
             <NuxtLink to="/profile" class="text-gray-500 hover:text-gray-700 px-4 py-2 font-medium flex items-center gap-2">
                Volver a Usuario
             </NuxtLink>
          </div>
        </div>
          ...
          <div class="space-y-6">
            <div class="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
              <div class="flex items-center gap-4 mb-4">
                <img v-if="business.logo" :src="getImageUrl(business.logo)" class="w-16 h-16 rounded-full object-cover border border-gray-200" />
                <div v-else class="w-16 h-16 rounded-full bg-gray-100 flex items-center justify-center text-gray-400">
                  <Store :size="24" />
                </div>
                <div>
                  <p class="font-bold text-gray-800">Estado</p>
                  <span v-if="business.verification_status === 'verified'" class="text-green-600 text-sm font-medium flex items-center gap-1">
            ...
            <div class="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
              ...
              <ul class="space-y-2">
                <li>
                  <button class="w-full text-left p-3 rounded bg-gray-50 text-gray-400 cursor-not-allowed flex items-center gap-2">
                    <Package :size="18" /> Productos (Próximamente)
                  </button>
                </li>
                <li>
                  <button class="w-full text-left p-3 rounded bg-gray-50 text-gray-400 cursor-not-allowed flex items-center gap-2">
